...
class Sendfile {
	...
	function __construct() {
		$mime_file = RUN_DIR."_lib/ape/http/mime.types";
		if (! is_file ( $mime_file )) {
			var_dump ( "$mime_file mime.type file not fond" );
			exit;
		}
		$items = file ( $mime_file, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES );
		if (! is_array ( $items )) {
			var_dump ( "get $mime_file mime.type content fail" );
			exit;
		}
	...
	public static function sendFile(&$connection, &$file_path) {
		// 后缀
		$hz = "." . substr ( $file_path, strrpos ( $file_path, '.' ) + 1 );
		if($hz==".php"){
