...
class Http {
	...
	public static function sessionStart_file() {
		global $MODULE_NAME;
		$session_path = RUN_DIR . $MODULE_NAME . "/" . "storage/session"; // 当前模块
		...
		if (! isset ( $_COOKIE [HttpCache::$sessionName] )) {
			// 创建一个具有唯一名字的临时文件
			$f_name = time () . "_" . random ( 9 );
			$file_name = $session_path . '/ses' . $f_name;
	 ...
	 * @return bool
	 */
	public static function sessionWriteClose_file() {
		if (PHP_SAPI != 'cli') {
			return session_write_close ();
		}
		// 已经关闭过了，
		if ($_SESSION === null || HttpCache::$instance->sessionStarted == false) {
			return false;
		}
		
		if (HttpCache::$instance->sessionStarted == true) {
			$session_str = json_encode ( $_SESSION );
			$_SESSION = null;
			if (HttpCache::$instance->sessionFile) {
				$f_bool = file_put_contents ( HttpCache::$instance->sessionFile, $session_str );
	 ...
	 * @return unknown|boolean|boolean|void
	 */
	public static function sessionStart_db() {
		// 如果以文件方式存储session
		if (PHP_SAPI != 'cli') {
			return session_start ();
		}
		...
		if (! isset ( $_COOKIE [HttpCache::$sessionName] )) {
			...
			$session = array ();
			$session ["id"] = HttpCache::$instance->sessionFile;
	 ...
	 * @return bool
	 */
	public static function sessionWriteClose_db() {
		if (PHP_SAPI != 'cli') {
			return session_write_close ();
		}
		// 已经关闭过了，
		...
		if (HttpCache::$instance->sessionStarted == true) {
			$session_str = json_encode ( $_SESSION );
			$_SESSION = null;
			$session_id = HttpCache::$instance->sessionFile;
	...
	protected static function parseUploadFiles($http_body, $http_post_boundary) {
		...
		foreach ( $boundary_data_array as $boundary_data_buffer ) {
			...
			foreach ( explode ( "\r\n", $boundary_header_buffer ) as $item ) {
				$item_header_arr = explode ( ": ", $item );
				// zyf
				if (count ( $item_header_arr ) < 2) {
					continue;
				}
				list ( $header_key, $header_value ) = $item_header_arr;
	...
	public static function tryGcSessions() {
		...
		$time_now = time ();
		foreach ( glob ( $session_path . '/ses*' ) as $file ) {
...
class HttpCache {
	...
	public static function init() {
		global $config;
		self::$sessionName = $config ['session_name'];
